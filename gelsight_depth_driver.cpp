...
#include <GL/glut.h> // render. copied from http://www.cc.gatech.edu/~jarek/courses/openGL/viewer.htm
...
struct PointNormal {float x[3];   float n[3]; };
struct Triangle {   PointNormal v[3];  };
...
void MyIdleFunc(void) { 
  if (OutputImageNum == NumFramesToShow) {
    exit(0);
  }
...
int main( int argc, const char *argv[] )
{
  ...
  // generate non-const arg list
  char ** argv_nonconst;
  argv_nonconst = (char **) malloc( sizeof(char *) * argc);
  for (int i=0; i<argc; i++){
    argv_nonconst[i] = (char *) malloc( strlen(argv[i]) * sizeof(char) );
    strcpy(argv_nonconst[i], argv[i]);
  }

  glutInit(&argc, argv_nonconst);
  InitGL();
  InitMenu();
  InitGeometry();
